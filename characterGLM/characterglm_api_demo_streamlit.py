...
def update_api_key(key: Optional[str] = None):
    ...
    #key = key or if st.session_state["API_KEY"]
    if key:
        api.API_KEY = key

# 设置API KEY
api_key = st.sidebar.text_input("API_KEY", value=os.getenv("ZHIPU_API_KEY", ""), key="API_KEY", type="password", on_change=update_api_key)
...
style_option = st.sidebar.selectbox("选择图片风格", ["二次元风格", "写实风格", "水墨画"], index=0, key="style_option")
...
def draw_new_image():
    ...
    image_prompt = style_option + '。' + image_prompt.strip()
